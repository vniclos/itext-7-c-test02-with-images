...
namespace testudines_pdf_test
{
    ...
    {
        String m_Dir_Resources_test="";
        String m_Dir_Temp = "";
        String m_Dir_Target = "";
        public frmTest()
        {
            InitializeComponent();
            m_Dir_Resources_test = (AppDomain.CurrentDomain.BaseDirectory+"resources-test/").Replace("\\", "/");
            m_Dir_Temp = (System.IO.Path.GetTempPath() + "/pdf/tmp/").Replace("\\","/");
            m_Dir_Target= (AppDomain.CurrentDomain.BaseDirectory + "/pdf/Target").Replace("\\", "/");
            scnDirTemp.Text = m_Dir_Temp;
            scnDirTarget.Text = m_Dir_Target;
